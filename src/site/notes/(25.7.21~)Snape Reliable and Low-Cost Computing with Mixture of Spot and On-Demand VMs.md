---
{"dg-publish":true,"permalink":"/25-7-21-snape-reliable-and-low-cost-computing-with-mixture-of-spot-and-on-demand-v-ms/"}
---
...
**be（is/are）+ being + 过去分词（utilized）**
	...
	主动句（主动进行）They are using the server.（他们正在使用服务器）
...
| 谓语动词短语       | began to drop                  | 动词 + 不定式        | 开始下降               |
...
| ↳ support a static mixture ratio | 动词 + 宾                              | 支持固定比例             |              |
...
| 方法状语                                   | by leveraging the fact that...                                       | 介词短语                                  | 通过利用……这一事实                |
...
1. 预测 Spot 实例的回收行为非常困难，因为必须研究多个导致回收的因素，这些因素既包括全局层面也包括本地层面。 
...
| ↳ if capacity permits                          | 条件状语从句                                  | 如果容量允许             |                 |
| 转折副词                                           | However                                 | 副词                 | 然而              |
